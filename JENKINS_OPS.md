# Jenkins 自动部署操作手册

## 目录
1. [快速开始](#快速开始)
2. [Jenkins基础配置](#jenkins基础配置)
3. [凭据管理](#凭据管理)
4. [创建部署任务](#创建部署任务)
5. [配置GitHub Webhook](#配置github-webhook)
6. [服务器初始化](#服务器初始化)
7. [首次构建](#首次构建)
8. [日常操作](#日常操作)
9. [问题排查](#问题排查)

---

## 快速开始

### 环境准备

- ✅ Jenkins已安装并运行
- ✅ 服务器IP：`106.15.90.212`
- ✅ 服务器用户：`root`
- ✅ GitHub仓库：`https://github.com/lpf2025/smart_order_hub.git`
- ✅ 需要准备：服务器root密码

---

## Jenkins基础配置

### 1.1 登录Jenkins

1. 打开浏览器
2. 访问Jenkins地址：`http://<Jenkins服务器IP>:8080`
3. 输入用户名和密码登录

### 1.2 安装必要插件

1. 点击左侧"Manage Jenkins"
2. 点击"插件管理"
3. 搜索并安装以下插件：
   - **Git Plugin**
   - **Pipeline Plugin**
   - **SSH Agent Plugin**
   - **Email Extension Plugin**
   - **Timestamper Plugin**
4. 安装完成后重启Jenkins

### 1.3 配置全局工具

1. 进入"Manage Jenkins" → "Global Tool Configuration"

#### 配置JDK
1. 找到"JDK"部分
2. 点击"JDK安装"
3. 填写：
   - Name：`JDK11`
   - JAVA_HOME：`/usr/lib/jvm/java-11-openjdk`
   - Java版本：`11.0.x`
4. 点击"保存"

#### 配置Maven
1. 在同一页面找到"Maven"部分
2. 点击"Maven安装"
3. 填写：
   - Name：`Maven3.8`
   - MAVEN_HOME：`/opt/maven`
   - Maven版本：`3.8.6`
4. 点击"保存"

#### 配置Node.js
1. 在同一页面找到"Node.js"部分
2. 点击"Node.js安装"
3. 填写：
   - Name：`Node18`
   - 自动安装：勾选
   - Node.js版本：`18.x`
4. 点击"保存"

---

## 凭据管理

### 2.1 添加服务器密码凭据

1. 进入"Manage Jenkins" → "Manage Credentials"
2. 点击"Global credentials (unrestricted)"
3. 点击"Add Credentials"
4. 填写信息：
   - **类型**：选择"Username with password"
   - **Scope**：选择"Global (Jenkins, nodes, items, all child items, etc)"
   - **Username**：`root`
   - **Password**：输入您的服务器root密码
   - **ID**：`server-password`
   - **Description**：`生产服务器root密码`
5. 点击"Create"

### 2.2 添加GitHub凭据

1. 在同一页面继续点击"Add Credentials"
2. 填写信息：
   - **类型**：选择"Username with password"
   - **Scope**：选择"Global"
   - **Username**：`624140273@qq.com`
   - **Password**：`135138785322qw`
   - **ID**：`github-credentials`
   - **Description**：`GitHub账号密码`
3. 点击"Create"

### 2.3 添加Webhook密钥

1. 在同一页面继续点击"Add Credentials"
2. 填写信息：
   - **类型**：选择"Secret text"
   - **Scope**：选择"Global"
   - **Secret**：`smart-order-hub-webhook-secret`
   - **ID**：`github-webhook-secret`
   - **Description**：`GitHub Webhook密钥`
3. 点击"Create"

**重要提示**：请记住这个密钥`smart-order-hub-webhook-secret`，配置GitHub webhook时需要使用

---

## 创建部署任务

### 3.1 创建新任务

1. 返回Jenkins首页
2. 点击左侧"新建任务"
3. 输入任务名称：`smart-order-hub-prod`
4. 选择任务类型：点击"流水线（Pipeline）"
5. 点击"确定"

### 3.2 配置General

在任务配置页面：

1. **描述**：
   ```
   智能点餐系统 - 生产环境自动部署
   ```

2. **丢弃旧的构建**：
   - 勾选"Discard old builds"
   - 保持构建个数：`10`
   - 天数：`30`

### 3.3 配置构建触发器

#### 选项A：GitHub webhook触发（推荐）

1. 在"构建触发器"部分
2. 勾选"GitHub hook trigger for GITScm polling"
3. **GitHub hook URL**：`http://<Jenkins公网IP或域名>/github-webhook/`
   - 示例：`http://106.15.90.212:8080/github-webhook/`
4. **Secret**：`smart-order-hub-webhook-secret`（与凭据中的ID一致）
5. 点击"保存"

#### 选项B：定时构建（可选）

如果需要定时构建：

1. 勾选"Build periodically"
2. 输入Cron表达式：
   - `H/30 * * * *` - 每30分钟构建一次
   - `H 2 * * *` - 每天凌晨2点构建
   - `H H(9-18) * * 1-5` - 工作日每小时构建

### 3.4 配置Pipeline

在"Pipeline"部分：

1. **Definition**：选择"Pipeline script from SCM"
2. **SCM**：选择"Git"
3. **Repository URL**：`https://github.com/lpf2025/smart_order_hub.git`
4. **Credentials**：选择`github-credentials`（如果没有，点击"Add"添加）
5. **Branches to build**：`*/main`
6. **Script Path**：`Jenkinsfile`
7. **Lightweight checkout**：勾选（加快检出速度）

### 3.5 保存配置

1. 滚动到页面底部
2. 点击"保存"按钮
3. 点击"应用"按钮

---

## 配置GitHub Webhook

### 4.1 进入GitHub仓库设置

1. 访问：https://github.com/lpf2025/smart_order_hub/settings/hooks
2. 点击"Add webhook"按钮

### 4.2 配置Webhook

1. **Payload URL**：
   ```
   http://<Jenkins公网IP或域名>/github-webhook/
   ```
   - 示例：`http://106.15.90.212:8080/github-webhook/`

2. **Content type**：选择`application/json`

3. **Secret**：
   ```
   smart-order-hub-webhook-secret
   ```
   - 必须与Jenkins凭据中的Secret完全一致

4. **Which events would you like to trigger this webhook?**
   - 勾选"Just the push event"
   - 勾选"Branch or tag creation"

5. **Active**：勾选"Active"

6. 点击"Add webhook"按钮

### 4.3 验证Webhook

1. 在webhook列表中查看刚创建的webhook
2. 点击"..."查看最近交付记录
3. 应该显示"200 OK"状态

---

## 服务器初始化

### 5.1 上传初始化脚本

在您的本地电脑执行：

```bash
# 1. 上传初始化脚本到服务器
scp scripts/server-init.sh root@106.15.90.212:/tmp/
```

### 5.2 执行初始化脚本

```bash
# 2. SSH登录到服务器
ssh root@106.15.90.212

# 3. 执行初始化脚本
bash /tmp/server-init.sh
```

### 5.3 验证初始化

初始化脚本会自动完成以下操作：

1. ✅ 更新系统
2. ✅ 安装Java 11
3. ✅ 安装Maven 3.8
4. ✅ 安装Node.js 18
5. ✅ 配置npm淘宝镜像
6. ✅ 安装sshpass
7. ✅ 创建部署目录
8. ✅ 配置防火墙
9. ✅ 配置Nginx
10. ✅ 重启Nginx服务

验证命令：

```bash
# 检查Java版本
java -version

# 检查Maven版本
mvn -version

# 检查Node.js版本
node -v

# 检查Nginx状态
systemctl status nginx

# 检查防火墙
firewall-cmd --list-all
```

---

## 首次构建

### 6.1 手动触发构建

1. 返回Jenkins首页
2. 找到任务"smart-order-hub-prod"
3. 点击"立即构建"按钮

### 6.2 查看构建日志

1. 点击构建号进入构建详情页
2. 点击"Console Output"标签查看实时日志

### 6.3 构建流程说明

Jenkins会自动执行以下步骤：

#### 阶段1：代码检出
- 从GitHub检出代码
- 检出main分支最新代码

#### 阶段2：环境检查
- 检查Java版本
- 检查Maven版本
- 检查Node.js版本
- 检查npm版本

#### 阶段3：后端打包
- 进入backend目录
- 执行`mvn clean package -DskipTests`
- 生成jar包：`backend/target/smart-order-hub-1.0.0.jar`

#### 阶段4：前端打包
- 进入pc-admin目录
- 执行`npm install`
- 执行`npm run build`
- 生成静态文件：`pc-admin/dist/`

#### 阶段5：部署后端
- 使用SCP上传jar包到服务器
- 上传启动脚本到服务器
- 停止旧服务
- 启动新服务

#### 阶段6：部署前端
- 使用SCP上传静态文件到服务器
- 清空旧文件
- 设置文件权限

#### 阶段7：健康检查
- 等待30秒让服务启动
- 检查后端API：`http://106.15.90.212:8081/api/health`
- 检查前端服务：`http://106.15.90.212/admin/`

#### 阶段8：清理构建
- 清理Jenkins工作空间

### 6.4 验证部署

构建成功后，访问以下地址验证：

1. **前端管理后台**：
   ```
   http://106.15.90.212/admin/
   ```
   - 应该能看到登录页面

2. **后端API健康检查**：
   ```
   http://106.15.90.212:8081/api/health
   ```
   - 应该返回JSON数据：
     ```json
     {
       "code": 200,
       "data": {
         "status": "UP",
         "timestamp": "2026-01-20T10:00:00",
         "application": "smart-order-hub",
         "version": "1.0.0"
       }
     }
     ```

---

## 日常操作

### 7.1 代码提交后自动部署

配置了GitHub webhook后，每次推送代码到main分支：

1. 开发人员提交代码
2. 推送到GitHub
3. GitHub自动触发Jenkins构建
4. Jenkins自动执行打包和部署
5. 部署完成后发送邮件通知

### 7.2 手动触发构建

如果需要手动部署：

1. 进入Jenkins首页
2. 找到任务"smart-order-hub-prod"
3. 点击"立即构建"
4. 查看构建日志

### 7.3 查看构建历史

1. 进入任务页面
2. 点击"Build History"
3. 查看所有构建记录
4. 点击构建号查看详细日志

### 7.4 查看构建状态

构建状态说明：

- 🔵 **蓝色**：构建中
- 🔵 **红色**：构建失败
- 🔴 **黄色**：构建不稳定
- 🟢 **绿色**：构建成功

### 7.5 查看构建时间

- **构建时间**：显示每个阶段的耗时
- **总耗时**：显示整个构建过程的总时间
- **平均时间**：可以查看历史构建的平均时间

---

## 问题排查

### 8.1 构建失败

#### 问题：Maven依赖下载失败

**错误信息**：
```
[ERROR] Failed to execute goal on project smart-order-hub: Could not resolve dependencies
```

**解决方案**：
```bash
# 清理Maven缓存
rm -rf ~/.m2/repository

# 使用阿里云镜像（已在初始化脚本中配置）
# 重新触发构建
```

#### 问题：npm install失败

**错误信息**：
```
npm ERR! network timeout
```

**解决方案**：
```bash
# 清理npm缓存
npm cache clean --force

# 使用淘宝镜像（已在初始化脚本中配置）
npm install --registry=https://registry.npm.taobao.org
```

#### 问题：代码检出失败

**错误信息**：
```
ERROR: Error fetching remote repo 'origin'
```

**解决方案**：
1. 检查GitHub凭据是否正确
2. 检查网络连接
3. 检查仓库URL是否正确
4. 重新触发构建

### 8.2 部署失败

#### 问题：SSH连接失败

**错误信息**：
```
Permission denied (publickey,password)
```

**解决方案**：
```bash
# 检查凭据ID是否正确
# 在Jenkins中检查server-password凭据

# 测试SSH连接
ssh root@106.15.90.212 "echo '连接成功'"

# 检查防火墙
firewall-cmd --list-all
```

#### 问题：端口被占用

**错误信息**：
```
Port 8081 is already in use
```

**解决方案**：
```bash
# 查看占用进程
lsof -ti:8081

# 停止进程
kill -9 <PID>

# 或者使用启动脚本自动处理
cd /opt/smart-order-hub/backend
./stop.sh
```

#### 问题：文件上传失败

**错误信息**：
```
scp: No such file or directory
```

**解决方案**：
1. 检查构建是否成功
2. 检查jar包是否生成
3. 检查dist目录是否存在
4. 重新触发构建

### 8.3 服务启动失败

#### 问题：Java内存不足

**错误信息**：
```
java.lang.OutOfMemoryError: Java heap space
```

**解决方案**：

修改[Jenkinsfile](file:///d:/git/smart_rder_hub/Jenkinsfile)中的启动命令：

```groovy
stage('部署后端') {
    steps {
        sh """
            # 增加JVM内存参数
            nohup java -Xms512m -Xmx1024m \\
                -jar $JAR_NAME \\
                --spring.profiles.active=prod \\
                > /var/log/smart-order-hub/app.log 2>&1 &
        """
    }
}
```

#### 问题：数据库连接失败

**错误信息**：
```
Communications link failure
```

**解决方案**：
```bash
# 检查数据库连接
mysql -h rm-uf6fjt8k75xbfc1phso.mysql.rds.aliyuncs.com -u root -p

# 检查防火墙
firewall-cmd --list-all

# 检查数据库配置
cat /opt/smart-order-hub/backend/application-prod.yml
```

### 8.4 健康检查失败

#### 问题：后端API无法访问

**错误信息**：
```
curl: (7) Failed to connect
```

**解决方案**：
```bash
# 检查服务是否启动
ps aux | grep smart-order-hub

# 检查端口是否监听
netstat -tlnp | grep 8081

# 查看日志
tail -100 /var/log/smart-order-hub/app.log

# 手动重启服务
cd /opt/smart-order-hub/backend
./stop.sh
./start.sh
```

#### 问题：前端页面404

**错误信息**：
```
404 Not Found
```

**解决方案**：
```bash
# 检查Nginx配置
nginx -t

# 检查文件是否存在
ls -la /var/www/admin/

# 检查文件权限
ls -la /var/www/admin/index.html

# 重新加载Nginx
nginx -s reload
```

---

## 高级操作

### 9.1 回滚到上一版本

如果新版本有问题，可以快速回滚：

```bash
# SSH登录服务器
ssh root@106.15.90.212

# 进入部署目录
cd /opt/smart-order-hub/backend

# 恢复备份
mv smart-order-hub-1.0.0.jar.backup smart-order-hub-1.0.0.jar

# 重启服务
./stop.sh
./start.sh
```

### 9.2 查看实时日志

```bash
# 查看后端日志
tail -f /var/log/smart-order-hub/app.log

# 查看Nginx日志
tail -f /var/log/nginx/access.log
tail -f /var/log/nginx/error.log

# 查看系统日志
journalctl -u jenkins -f
```

### 9.3 清理旧构建

Jenkins会自动清理旧构建，但也可以手动清理：

1. 进入任务配置
2. 勾选"Discard old builds"
3. 设置保持构建个数：`10`
4. 设置天数：`30`

### 9.4 配置多环境

如果需要多环境部署（开发/测试/生产）：

1. 创建多个Jenkins任务：
   - `smart-order-hub-dev` - 开发环境
   - `smart-order-hub-test` - 测试环境
   - `smart-order-hub-prod` - 生产环境

2. 每个任务使用不同的配置：
   - 不同的服务器IP
   - 不同的部署路径
   - 不同的数据库

---

## 附录

### A. Jenkinsfile完整路径

```
https://github.com/lpf2025/smart_order_hub/blob/main/Jenkinsfile
```

### B. 服务器初始化脚本路径

```
https://github.com/lpf2025/smart_order_hub/blob/main/scripts/server-init.sh
```

### C. 后端启动脚本路径

```
https://github.com/lpf2025/smart_order_hub/blob/main/scripts/backend-start.sh
```

### D. 后端停止脚本路径

```
https://github.com/lpf2025/smart_order_hub/blob/main/scripts/backend-stop.sh
```

### E. 常用端口

| 服务 | 端口 | 用途 |
|------|------|------|
| Jenkins | 8080 | Jenkins Web界面 |
| 后端API | 8081 | 后端服务 |
| 前端 | 80 | HTTP访问 |
| SSH | 22 | 远程连接 |
| MySQL | 3306 | 数据库 |

---

## 快速参考

### 首次部署检查清单

- [ ] Jenkins已安装并运行
- [ ] 必要插件已安装
- [ ] 全局工具已配置
- [ ] 凭据已创建
- [ ] Jenkins任务已创建
- [ ] GitHub webhook已配置
- [ ] 服务器已初始化
- [ ] 首次构建成功
- [ ] 前端可以访问
- [ ] 后端API正常

### 日常部署检查清单

- [ ] 代码已提交到GitHub
- [ ] Jenkins构建已触发
- [ ] 构建成功
- [ ] 部署成功
- [ ] 健康检查通过

---

## 总结

按照本文档操作完成后，您将拥有：

✅ **完全自动化的部署流程**
✅ **一键完成打包、上传、部署**
✅ **版本控制和回滚能力**
✅ **实时监控和邮件通知**
✅ **详细的构建日志和历史记录**

所有配置文件已推送到GitHub，可以直接使用！
